...
def run_simulation(net_file_path: str, end_time: int = 500, use_gui: bool = False,
    ...
    if rl_config is None:
        rl_config = {}
    if given_tls_dict:
        with open(os.path.join(checkpoint_dir, 'tls_dict.pkl'), 'rb') as f:
            given_tls_dict_ = pickle.load(f)
    else:
        given_tls_dict_ = None
    ltf = LTF_Traci(
        ...
        need_dump_tlc=need_dump_tlc,
        real_flow_file=real_flow_file,
        tls_to_train=tls_to_train,
        partial_train=partial_train,
        given_tls_dict=given_tls_dict_
    )

    try:
        if tls_to_train is not None:
            tl_phase_result, lane_averages = ltf.run_partial_training(tls_to_train=tls_to_train)
        else:
...
class OptimizationRunner:
    ...
    def run_optimization_simulation(self, net_file_path: str, route_file: str = None,
                                  rl_tls_ids: list = None, tls_to_train: list = None, **kwargs):
        """
        运行优化仿真
        ...
        real_flow_file = self.data_paths.get('real_flow_file', None)
        # 合并参数
        sim_params = {
            ...
            'need_dump_tlc': self.config.get('need_dump_tlc', False),
            'partial_train': self.config.get('partial_train', False),
            'given_tls_dict': self.config.get('given_tls_dict', False),
        }
