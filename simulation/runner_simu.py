...
class SimulationRunner:
    ...
    def run_simulation_with_traci(self, 
        ...
        # 1. 对车辆按出发时间排序
        sorted_rou_file,numcars = self._sort_vehicles_by_depart_time(rou_file)
    ...
    def _sort_vehicles_by_depart_time(self, rou_file: str) -> str:
        ...
        count_car=sort_vehicles_by_depart_time(rou_file, rou_file)
        
        # 检查文件大小
        file_size = os.path.getsize(rou_file)
        print(f"排序后文件大小: {file_size} 字节")
        
        return rou_file,count_car
    ...
    def update_rouxml(self,changedict):
        if self.stop_add==False:
            vehicle_elements = self.generate_trips_and_extract_vehicles(self.need_add_road, self.net_file, 25, 'io.xml')
