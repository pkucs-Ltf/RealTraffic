...
class SimuTask:
    ...
    def copy_file(self,source_path, destination_path):
        ...
        - 如果源文件不存在，则会先创建一个空的源文件。
        - 如果目标文件的目录不存在，则会自动创建该目录。

        参数:
        source_path (str): 要复制的源文件的路径。
        destination_path (str): 目标文件的路径。
        """
        try:
            # 检查源文件是否存在，如果不存在则创建一个空的
            if not os.path.exists(source_path):
                print(f"源文件 {source_path} 不存在，将创建一个新文件。")
                # 使用 'w' 模式打开文件会自动创建它
                with open(source_path, 'w') as f:
                    pass  # 创建一个空文件

            # 确保目标目录存在
            # os.path.dirname() 可以获取文件的目录路径
            dest_dir = os.path.dirname(destination_path)
            
            # 如果目录路径不为空且目录不存在，则创建它
            ...
            print(f"文件已成功从 {source_path} 复制到 {destination_path}")
            return True
