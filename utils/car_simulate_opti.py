...
class LTF_Traci:
    ...
    def __init__(self, net_file, route_file, use_gui=True, 
                 end_time=3600, yellow_time=3, t_min=12, seed=42,need_optimize_tlds=[],given_tlds_type='road',part_optimize=False,sumobinary='sumo',
                 # 新增RL参数（向后兼容）
                 ...
                 need_dump_tlc: bool = False,
                 tls_to_train: list = None,
                 partial_train: bool = False,
                 real_flow_file: str = None,
                 given_tls_dict: dict = None):  # 训练回合数
        """
        初始化LTF_Traci对象
        ...
        self.real_flow_file = real_flow_file
        self.use_gui = False
        ...
        phase_control_cfg = self.rl_config.get('phase_control', {})
        self.phase_control_config = phase_control_cfg
        self.min_phase_duration = max(int(phase_control_cfg.get('min_green', 3)), 1)
        self.max_phase_duration = max(int(phase_control_cfg.get('max_green', 40)), self.min_phase_duration + 1)
        self.pressure_normalizer = float(phase_control_cfg.get('pressure_normalizer', 20.0))
        self.pressure_reward_weight = float(phase_control_cfg.get('pressure_reward_weight', 1.0))
        self.pressure_advantage_weight = float(phase_control_cfg.get('pressure_advantage_weight', 0.3))
        self.duration_penalty_weight = float(phase_control_cfg.get('duration_penalty_weight', 0.1))
        self.duration_penalty_threshold = int(phase_control_cfg.get('duration_penalty_threshold', max(self.max_phase_duration - 10, self.min_phase_duration)))
        self.switch_penalty = float(phase_control_cfg.get('switch_penalty', 0.05))
        self.forced_switch_penalty = float(phase_control_cfg.get('forced_switch_penalty', 0.3))
        self.early_switch_penalty = float(phase_control_cfg.get('early_switch_penalty', 0.02))
    ...
    def _init_connection_train(self):
        ...
        sumo_binary = 'sumo'
        if self.use_test:
            route_file = self.real_flow_file
        else:
    ...
    def _init_rl_controllers(self):
        ...
        try:
            self.controller_manager = TrafficLightControllerManager(
                ...
                ltf_instance=self,
                need_dump_tlc=self.need_dump_tlc,
                given_tls_dict=self.given_tls_dict
            )
            if self.rl_tls_ids:
            ...
            if self.partial_train:
                self.controller_manager.load_checkpoint_partial(self.tls_to_train)
   ...
   # 移到 if phases 内部
        ...
        for tl_id in self.tls_ids:
            ...
            if tl_id in self.intersections:
                ...
                if self.controller_manager is not None:
                    target_phase = self.controller_manager.decide_action(tl_id, observation, step, is_training_agent=False)
                else:
                    # 回退到原有贪心逻辑
        ...
        if is_decision_step:
            ...
            for tl_id in self.rl_tls_ids:
                if self.total_decision_num > self.learning_start:
                    # 使用学习到的策略
                    action = self.controller_manager.decide_action(tl_id, current_observations.get(tl_id, {}), step, is_training_agent=True)
                else:
                    # 初期随机探索
            ...
            # 5.1 记录本次实际执行的动作（以当前相位为准）
            try:
                self.last_executed_actions = {tl_id: self.current_phases.get(tl_id, 0) for tl_id in self.rl_tls_ids}
            except Exception:
                self.last_executed_actions = current_actions.copy()
            
            if len(self.rl_tls_ids)!=0:
                # 6. 存储当前状态和"实际执行的动作"，用于下次经验拼接
                self.last_observations = current_observations.copy()
                self.last_actions = self.last_executed_actions.copy()
    ...
    def _apply_rl_actions_test(self, step):
        if step==0:
            self.dict_record_action={}
        """强化学习控制模式（加载已有权重的测试模式）"""
        # 检查是否到了决策时刻
        is_decision_step = (step % self.action_interval == 0)
        ...
        if is_decision_step:
            ...
            if len(self.rl_tls_ids) != 0:
                # 1. 获取当前状态观测
                # if step==15 and tls_id=='cluster_11516114382_12541569785_3781490684_49789275_#3more':
                #     a=1
                if step==15:
                    a=1
                current_observations = self._get_traffic_observations()
                ...
                for tl_id in self.rl_tls_ids:
                    # 直接使用学习到的策略进行决策
                    action = self.controller_manager.decide_action(tl_id, current_observations.get(tl_id, {}), step, is_training_agent=False)
                    current_actions[tl_id] = action
            ...
            # 4. 应用动作到所有交通灯
            self._apply_actions_to_traffic_lights(step, current_actions)
            # 4.1 记录实际执行的动作，并用于回放/分析
            try:
                self.last_executed_actions = {tl_id: self.current_phases.get(tl_id, 0) for tl_id in self.rl_tls_ids}
            except Exception:
                self.last_executed_actions = current_actions.copy()
            # 若需要记录动作轨迹，记录实际执行而非原始决策
            self.dict_record_action[step] = self.last_executed_actions.copy()
        ...
        # 在测试模式下，仍然收集奖励用于评估
        if len(self.rl_tls_ids) != 0 and step % self.cal_reward_circle == 0:
            step_rewards = self._calculate_rewards()
            for tl_id, reward in step_rewards.items():
                self.reward_accumulator[tl_id].append(reward)
    ...
    def _apply_rl_actions_partial_training(self, step, tls_to_train: List[str]):
        """强化学习控制模式（按action_interval决策），对部分信号灯进行训练，其余使用测试模式"""
        is_decision_step = (step % self.action_interval == 0)
        ...
        if is_decision_step:
            ...
            # 2. 仅为训练中的信号灯存储经验
            if self.total_decision_num > 0 and self.last_observations:
                self._store_experience_batch_partial(current_observations, step, tls_to_train)
            
            # 3. 决策新动作
            current_actions = {}
            for tl_id in self.rl_tls_ids:
                if tl_id in tls_to_train:
                    # 训练模式：带探索
                    if self.total_decision_num > self.learning_start:
                        action = self.controller_manager.decide_action(tl_id, current_observations.get(tl_id, {}), step, is_training_agent=True)
                    else:
            ...
            # 7. 仅为训练中的信号灯更新模型
            self._check_and_update_models_partial(tls_to_train)
    ...
    def _apply_actions_to_traffic_lights(self, step, actions_dict):
        ...
        for tl_id in self.tls_ids:
            if tl_id in self.intersections:
                ...
                if step % 5==0 and self.tra_pressure:
    ...
    def _store_experience_batch_partial(self, current_observations, current_step, tls_to_train: List[str]):
        """批量存储经验到经验回放缓冲区（部分训练）"""
        if not self.reward_accumulator:
            return
        ...
        for tl_id in tls_to_train: # 只为训练中的信号灯存储经验
            if tl_id in self.last_observations and tl_id in self.last_actions:
                transition = {
                    'state': self.last_observations[tl_id],
                    'action': self.last_actions[tl_id],
                    'reward': avg_rewards.get(tl_id, 0.0),
                    'next_state': current_observations.get(tl_id, {}),
                    'done': current_step >= self.end_time
                }
    ...
    def _check_and_update_models_partial(self, tls_to_train: List[str]):
        """检查并执行模型更新（部分训练）"""
        if self.total_decision_num < 100:
            return
        ...
        if self.total_decision_num % self.update_model_rate == 0:
            print(f"执行模型学习更新 (决策次数: {self.total_decision_num})")
            for tl_id in tls_to_train: # 只为训练中的信号灯更新
                if tl_id in self.controller_manager.controllers:
                    controller = self.controller_manager.controllers[tl_id]
                    if hasattr(controller, 'agent'):
                        metrics = controller.agent.learn()
        ...
        if self.current_phase_times[tl_id] >= self.t_min:
            ...
            if target_phase != current_phase:
                ...
                if phase_index != -1:
                    ...
                    self.conn.trafficlight.setPhaseDuration(tl_id, 60)
    ...
    def run(self, need_add=[]):
        ...
        for episode in range(self.num_episodes):
            ...
            self.tra_pressure=False
            self.use_test=True
            _,_,test_metrics_history_all=self._run_single_episode_test()
            print('test',test_metrics_history_all)
            self.use_test=False
            self.tra_pressure=True
            
              
            if test_metrics_history_all[-1]['arrival_carnum']>max_arrnum:
                max_arrnum=test_metrics_history_all[-1]['arrival_carnum']
                ...
                print('test_mode',test_metrics_history_all)
                with open(self.reward_static_pkl, 'wb') as f:
                    pickle.dump([test_metrics_history_all], f)   
    ...
    def run_partial_training(self, tls_to_train: List[str], need_add=[]):
        """多回合部分训练循环"""
        print(f"开始多回合部分RL训练，共 {self.num_episodes} 回合")
        print(f"训练中的信号灯: {tls_to_train}")
        print(f"RL模式: {self.rl_mode}")
        ...
        for episode in range(self.num_episodes):
            self.n_episodes = episode
            print(f"\n=== 第 {episode + 1}/{self.num_episodes} 回合 (部分训练) ===")
            if episode != 0:
                self._init_connection_train()
            
            # 运行单回合部分训练
            episode_rewards, metrics_history, metrics_history_all = self._run_single_episode_partial_training(episode, tls_to_train, need_add)
            if len(tls_to_train) == 0:
                with open(self.reward_static_pkl, 'wb') as f:
                    pickle.dump([metrics_history_all], f)
                break
            ...
            self.tra_pressure=True
            # 记录统计信息
            if test_metrics_history_all:
                all_metrics_new.append(test_metrics_history_all) 


            # 根据到达车辆数决定是否保存检查点
            if test_metrics_history_all and max_arrnum < test_metrics_history_all[-1]['arrival_carnum']:
                max_arrnum = test_metrics_history_all[-1]['arrival_carnum']
                print('episode',episode,'max_arrnum',max_arrnum,test_metrics_history_all[-1])
                if self.controller_manager is not None:
                    self.controller_manager.save_checkpoint_partial(episode, tls_to_train)
                    print(f"已保存第 {episode} 回合的部分训练检查点")
    ...
    def _run_single_episode_partial_training(self, episode_id: int, tls_to_train: List[str], need_add=[]):
        """单回合部分训练仿真"""
        # 重置训练相关变量
        if not hasattr(self, 'total_decision_num'):
            self.total_decision_num = 0
        ...
        metrics_history = []
        # if len(tls_to_train) == 0:
        #     self.controller_manager.set_eval_mode()
        
        while step < self.end_time:
            if step % 100 == 0:
                print(f"第 {step} 步 (部分训练)")
   
            # 应用部分训练的控制策略
            step = self._apply_rl_actions_partial_training(step, tls_to_train)
        ...
        if metrics_history:
            ...
            avg_metrics = {}
        # if len(tls_to_train) == 0:
        #     self.controller_manager.set_train_mode()
        return episode_rewards, avg_metrics, metrics_history

    def _run_single_episode_test(self):
        """运行单次仿真（测试模式）"""
        
        self._init_connection_train()
        # 切换控制器到评测模式，确保关闭Dropout/BN随机
        if self.controller_manager is not None:
            try:
                self.controller_manager.set_eval_mode()
            except Exception:
        ...
        self.last_observations = {}
        self.last_actions = {}
        self.reward_accumulator = defaultdict(list)
        self.last_decision_step = -1
        self.total_decision_num = 0
        # 确保action_interval已设置
        ...
        while step < self.end_time:
            ...
            step = self._apply_rl_actions_test(step)
    ...
    def _get_traffic_observations(self) -> Dict[str, Dict]:
        ...
        for tl_id in self.rl_tls_ids:
            ...
            # if step==15 and tl_id=='cluster_11516114382_12541569785_3781490684_49789275_#3more':
            #     observations[tl_id][4]=0.01 
